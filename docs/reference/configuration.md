...
Configuration for oh-my-opencode.

## Quick Start

Most users do not need to configure anything manually. Run the interactive installer:
...
**Want to customize?** Here are the common patterns:
...
| `unspecified-low`    | `anthropic/claude-sonnet-4-6`      | Tasks that do not fit other categories, low effort required           |
| `unspecified-high`   | `anthropic/claude-opus-4-6` (max)  | Tasks that do not fit other categories, high effort required          |
...
| `timeout_seconds`       | `30`                   | Timeout in seconds for an in-flight fallback request before forcing the next fallback model. **Set to `0` to disable auto-retry signal detection** (see below). |
...
**IMPORTANT**: Setting `timeout_seconds: 0` **disables auto-retry signal detection**. This is a critical behavior change:
...
| `timeout_seconds: 0` | **Limited fallback**: Only error-based fallback works. Provider retry messages are **completely ignored**. Timeout-based escalation is **disabled**. |
...
When `tmux.enabled` is `true` and you are inside a tmux session:
...
For convenience, create a shell function that automatically handles tmux sessions and port allocation. Here is an example for Fish shell:
...
Do not want them? Disable via `disabled_mcps` in `~/.config/opencode/oh-my-opencode.json` or `.opencode/oh-my-opencode.json`:
...
3. **Test with curl**:
   ...
   ```
4. **See detailed troubleshooting**: [Ollama Troubleshooting](../troubleshooting/ollama.md)
